...
public class SwiftVideoCompressPlugin: NSObject, FlutterPlugin {
    ...
    public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        guard let args = call.arguments as? [String: Any] else {
            result(FlutterError(code:"ARG_ERROR",message:"Invalid arguments",details:nil))
            return
        }
        switch call.method {
        ...
        case "compressVideo":
            guard  let path = args["path"] as? String,!path.isEmpty else {
                result(FlutterError(code: "PATH_ERROR", message: "Missing or empty video path", details: nil))
                return
            }
            let quality = args["quality"] as! NSNumber ?? 0
